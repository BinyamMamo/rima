...
import { MagnifyingGlass, Funnel, ChatCenteredText, Info, IdentificationCardIcon, ChatCenteredIcon } from "@phosphor-icons/react";
    ...
    <div className="w-full max-w-2xl px-6 flex flex-col gap-8 pb-32 pt-8 overflow-y-auto scrollbar-hide animate-fade-in">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search by name or roless..."
        ...
        {filtered.map(user => (
          ...
          >
            <div className="flex items-center gap-4 flex-1 min-w-0">
              <div className={`w-14 h-14 min-w-[3.5rem] min-h-[3.5rem] shrink-0 rounded-2xl ${user.avatarColor} flex items-center justify-center text-xl font-black shadow-lg relative`}>
                {getInitials(user.name)}
                <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-4 border-[var(--bg-card)] ${user.status === 'active' ? 'bg-[#3ECF8E]' : user.status === 'away' ? 'bg-[#F59E0B]' : 'bg-[#94A3B8]'
                  }`} />
              </div>
              <div className="flex flex-col min-w-0 flex-1">
                <span className="text-lg font-bold text-[var(--text-primary)] group-hover:text-[var(--primary)] transition-colors truncate">{user.name}</span>
                <span className="text-xs font-black uppercase tracking-widest text-[var(--text-muted)] truncate">{user.role}</span>
              </div>
            </div>
            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
              <button
                className="p-3 bg-[var(--bg-surface)] rounded-xl text-[var(--text-secondary)] hover:text-[var(--primary)] hover:bg-[var(--primary)]/10 transition-all"
                onClick={(e) => {
                  e.stopPropagation();
                  onParticipantClick(user);
                }}
              >
                <ChatCenteredIcon size={20} weight="bold" />
              </button>
              <button
                className="p-3 bg-[var(--bg-surface)] rounded-xl text-[var(--text-secondary)] hover:text-[var(--primary)] hover:bg-[var(--primary)]/10 transition-all"
                onClick={(e) => {
